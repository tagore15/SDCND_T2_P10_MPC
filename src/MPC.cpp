...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N-1; i++)
    {
        // we want to apply low steering and acceleration 
        fg[0] += 5*CppAD::pow(vars[delta_start+i], 2);
        fg[0] += 5*CppAD::pow(vars[a_start+i], 2);
    }
    for (int i = 0; i < N-2; i++)
    {
        // we don't want to apply sudden high steering or acceleration
    ...
    // initial position constraints are same as current position
    ...
    { // constraint on difference between expected and model postion
        ...
        // y coordinates is taken as CTE
        AD<double> f0 = coeffs[0] + coeffs[1]*x0 + coeffs[2]*x0*x0 + coeffs[3]* x0*x0*x0;

        // desired psi in previous state as slope of curve
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0; i < delta_start; i++) {
    // values for non actutators could be any number
    vars_lowerbound[i] = -1.0e19;
    vars_upperbound[i] = 1.0e19;
  }
  for (int i = delta_start; i < a_start; i++) {
    // values for steering in radian 
    vars_lowerbound[i] = -0.436332*Lf;
    vars_upperbound[i] = 0.436332*Lf;
  }
  for (int i = a_start; i < n_vars; i++) {
    // values for accleration (throttle) 
